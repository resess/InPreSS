...
import corex.empiricalstudy.recommendation.BreakerRecommender;
import corex.empiricalstudy.training.DED;
import corex.empiricalstudy.training.TrainingDataTransfer;
...
public class Simulator  {
	...
	protected TraceNode findObservedFault(TraceNode node, Trace buggyTrace, Trace correctTrace){
		...
		//System.out.println("debug node firstTearDownNode: "+firstTearDownNode);
...
//		System.out.println("debug: last observable "+ lastObservableFault);//assertion
		...
		if(lastObservableFault!=null){
			...
			observedFaultList.add(lastObservableFault);//just parents
...
//				System.out.println("debug: last getStepOverPrevious "+ node);
	...
	private TraceNode findAsserttion(Trace buggyTrace, String methodTest, String lineNo) {
	  for (int i = buggyTrace.getExecutionList().size()-1; i>=0; i--) {
		 TraceNode node = buggyTrace.executionList.get(i);
		 if(node.toString().contains("line "+lineNo+" in "+methodTest)) {
			 return node;
		 }
	  }
	  return null;
...
//		}
	...
	public TraceNode getObservedFault() {
		...
		return this.observedFaultList.get(0);
	}
