...
import corex.util.TraceNodeVariableSimilarityComparator;
...
//		TraceNode invocationParent = node.getInvocationParent();
	...
	public List<IndexTreeNode> getControlPath(){
		...
		//System.out.println("Debug: get control path for Node is " + this.getTraceNode());
		...
		while(parent != null && hasSameInvocationParent(this, parent)){
			//System.out.println("Debug: Node is " + this.getTraceNode() + " parent is " +parent.getTraceNode() );
			
			//System.out.println("parent is " +parent.getTraceNode());
			//if(isLoopControlBy(parent)){//debug: comment this
				path.add(parent);				
			//}
		...
		for(int i=path.size()-1; i>=0; i--){
			...
			//System.out.println("Debug: Node is " + this.getTraceNode() + " path is " +node );
...
//		p.setLoopScope(null);
	...
	public boolean isTheSameWith(GraphNode node) {
		//debug: the below if was commented.
