...
public CustomFormatter newFormatter(TreeContext ctx1,TreeContext ctx2) throws Exception {
    return new CustomFormatter(ctx1,ctx2);
}

public Map<Integer, String[]> getChanes() throws Exception {
    CustomFormatter fmt = this.newFormatter(context1,context2);
    Map<Integer,List<Action>> orderMap = orderActions(fmt);
...
public Map<Integer, String[]> getChanes2() throws Exception {
    CustomFormatter fmt = this.newFormatter(context1,context2);
...
public Map<Integer,List<Action>> orderActions(CustomFormatter fmt) throws Exception {
    Iterator var3 = this.actions.iterator();
...
private class CustomFormatter {
    ...
    public CustomFormatter(TreeContext ctx1, TreeContext ctx2) {
        this.context1 = ctx1;
        this.context2 = ctx2;
    }
...
//        System.out.println("Match:" + srcNode.getPos() + " " + destNode.getPos() + " " + this.toS(srcNode, context1) + " " + this.toS(destNode, context2));
    }
...
//        System.out.println("[Update,%d,%d,%s]" + src.getPos() + " " + dest.getPos() + " " +this.toS(src, context1));
...
//        System.out.println("Line : Insert " + iTree.getPos() + " " +this.toS(iTree, context2));
    }

    public void insertAction(ITree node, ITree parent) throws Exception {
    	insertActionAux(node, parent, context2);
//        System.out.println("[Insert,%d,%d,%s]" +  node.getPos()+ " " + node.getEndPos() + " " + this.toS(node, context2));
    }
